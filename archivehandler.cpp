...
int Archive::extract(const QString file_to)
{
  ...
  ext.a = archive_write_disk_new();
  archive_write_disk_set_options(ext.a, flags);
  archive_write_disk_set_standard_lookup(ext.a);
  ...
  for (;;) {
    ...
    r = archive_write_header(ext.a, entry);
    if (r < ARCHIVE_OK)
      qDebug() << archive_error_string(ext.a);
    ...
    r = archive_write_finish_entry(ext.a);
    if (r < ARCHIVE_OK)
      qDebug() << archive_error_string(ext.a);
    if (r < ARCHIVE_WARN){
        qDebug() << archive_error_string(a);
...
//  archive_read_close(a);
//  archive_read_free(a);
//  archive_write_close(ext.a);
//  archive_write_free(ext.a);
  return 0;
}


int
Archive::copyData(Archive aw)
{
  int r;
  const void *buff;
  size_t size;
  ...
  for (;;) {
    ...
    r = archive_write_data_block(aw.a, buff, size, offset);
    if (r < ARCHIVE_OK) {
      qDebug() << archive_error_string(aw.a);
      return r;
    }
